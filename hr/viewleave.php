...
if (mysqli_num_rows($sqlChecklist) > 0) {
    ...
    $spls = $checklist['spl'] ?? 0;
    $splused = $checklist['spl_used'] ?? 0;
} else {
    $vacation = $vlused = $sick = $slused = $pto = $ptoused = $bdleave = $bdused = $eaout = $eaused = $spls = $splused = "";
}
...
<div class="row">
    <div class="col-lg-12">
        <h4 style="text-indent: 10px;">
            <a href="?manageleave&page=<?= $page; ?>"><i class="fa fa-arrow-left"></i> BACK</a> 
...
<?php
  if (isset($_GET['submit'])) {
      ...
      if (mysqli_num_rows($sqlCheck) > 0) {
          $table = "leave_credits";
          $values = "SET vacationleave='$vecay',vlused='$vlused',sickleave='$sicky',slused='$slused',pto='$pty',ptoused='$ptoused',bdayleave='$bdayleave', blp_used='$blp_used', earlyout='$earlyout', eo_used='$eo_used', spl='$spl',  spl_used='$spl_used',updatedby='$addedby',updateddatetime='$datenow' WHERE idno='$idno'";
      ...
      if ($sqlAddEmployee) {
          echo "<script>";
          echo "alert('Details successfully saved!'); window.location='?viewleave&id=$id&page=$page';";
          echo "</script>";
      } else {
          echo "<script>";
          echo "alert('Unable to save details!'); window.location='?viewleave&id=$id&page=$page';";
          echo "</script>";
      }
  }
?>