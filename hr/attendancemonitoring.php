...
?>
            ...
            if (isset($_GET['deleteinfraction'])) {
              ...
              // Delete the infraction from the points table
              $sqlDelete = mysqli_query($con, "DELETE FROM points WHERE id='$id'");
              
              if ($sqlDelete) {
                  // Update the attendance table to set remarks to 'P'
                  $sqlUpdate = mysqli_query($con, "UPDATE attendance SET remarks = 'P' WHERE logindate='$logindate' AND idno='$idno'");
                  
                  echo "<script>";
                  echo "alert('Infraction successfully removed!');";
                  echo "window.history.back();";
                  echo "</script>";
              } else {
                  echo "<script>";
                  echo "alert('Unable to remove infraction!');";
                  echo "window.history.back();";
                  echo "</script>";
              }
          }
