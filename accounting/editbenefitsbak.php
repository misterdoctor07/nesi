...
<script type="text/javascript">
    ...
    <div class="row">
      <div class="col-lg-12">
      <h4 style="text-indent: 10px;"><a href="?managebenefits"><i class="fa fa-arrow-left"></i> BACK</a> | <i class="fa fa-file-text"></i> EMPLOYEE BENEFITS</h4>
    </div>
    </div>
  ...
  <?php
    if(isset($_GET['submit'])){
        ...
        if(mysqli_num_rows($sqlCheck)>0){
            ...
            $values="SET sss='$sss',phic='$phic',hdmf='$hdmf',salary='$salary',updatedby='$addedby',updateddatetime='$datenow' WHERE idno='$id'";
            $sqlAddEmployee=mysqli_query($con,"UPDATE $table $values");
        }else{
            $table="employee_payroll(idno,sss,phic,hdmf,salary,addedby,addeddatetime)";
            $values="VALUES('$idno','$sss','$phic','$hdmf','$salary','$addedby','$datenow')";
            $sqlAddEmployee=mysqli_query($con,"INSERT INTO $table $values");
        }
      if($sqlAddEmployee){
        echo "<script>";
          echo "alert('Details successfully saved!');window.location='?editbenefits&id=$id';";
        echo "</script>";
      }else{
        echo "<script>";
          echo "alert('Unable to saved details!');window.location='?editbenefits&id=$id';";
        echo "</script>";
      }
    }
  ?>
