                ...
                <table class="table table-bordered table-striped table-condensed" id="hidden-table-info">
                  <thead>
                    <tr>
                      ...
                      <th style="vertical-align:middle;">Life Insurance Effectivity</th>                      
                      <th style="vertical-align:middle;">HMO Effectivity</th>
                  ...
                  <tbody>
                    ...
                    mysqli_query($con,"SET NAMES 'utf8'");
                      $sqlEmployee=mysqli_query($con,"SELECT ep.*,ed.*,eb.* FROM employee_profile ep LEFT JOIN employee_details ed ON ed.idno=ep.idno INNER JOIN employee_benefits eb ON eb.idno=ep.idno WHERE ed.status NOT LIKE '%RESIGNED%' AND ed.company='$comp' AND (YEAR(eb.insurance) >= '".date('Y')."' OR YEAR(eb.hmo) >= '".date('Y')."') ORDER BY ep.lastname ASC");
                      if(mysqli_num_rows($sqlEmployee)>0){
                        while($company=mysqli_fetch_array($sqlEmployee)){
                          ...
                          echo "<tr>";
                            ...
                            echo "<td align='center'>$hmo</td></td>";
