...
if(mysqli_num_rows($sqlChecklist)>0){
    ...
    $slps=$checklist['slp']??0;
    $slpused=$checklist['slp_used']??0;
} else {
  $vacation="";
  $vlused="";
  $sick="";
...
$slps = $checklist['slp'] - $checklist['slp_used'];
...
<script type="text/javascript">
    ...
    <div class="row">
      <div class="col-lg-12">
      <h4 style="text-indent: 10px;"><a href="?manageleave"><i class="fa fa-arrow-left"></i> BACK</a> | <i class="fa fa-file-text"></i> EMPLOYEE LEAVE CREDITS</h4>      
    </div>
    </div>
    ...
    <div class="col-lg-4 mt">
            ...
            <div class="panel-body">                                            
                ...
                <div class="form-group">
                  <label class="col-sm-4 col-sm-4 control-label">SLP Credits</label>
                  <div class="col-sm-3">
                    <input type="text" class="form-control" name="slp" style="text-align:center;" value="<?=$slps;?>">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-4 col-sm-4 control-label">SLP Used</label>
                  <div class="col-sm-3">
                    <input type="text" class="form-control" name="slp_used" style="text-align:center;" value="<?=$slpused;?>">
                  </div>
                </div>
            </div>
          </div>
  ...
  <?php
    if(isset($_GET['submit'])){
        ...
        $slp=$_GET['slp'];
        $slp_used=$_GET['slp_used'];

        $sqlCheck=mysqli_query($con,"SELECT * FROM leave_credits WHERE idno='$idno'");
        if(mysqli_num_rows($sqlCheck)>0){
            $table="leave_credits";
            $values="SET vacationleave='$vecay',vlused='$vlused',sickleave='$sicky',slused='$slused',pto='$pty',ptoused='$ptoused',bdayleave='$bdayleave', blp_used='$blp_used', earlyout='$earlyout', eo_used='$eo_used', slp='$slp',  slp_used='$slp_used'

            ,updatedby='$addedby',updateddatetime='$datenow' WHERE idno='$idno'";
            $sqlAddEmployee=mysqli_query($con,"UPDATE $table $values");
        }else{
            $table="leave_credits(idno,vacationleave,vlused,sickleave,slused,pto,ptoused,bdayleave,blp_used,earlyout,eo_used,slp,slp_used,addedby,addeddatetime)";
            $values="VALUES('$idno','$vecay','$vlused','$sicky','$slused','$pty','$ptoused','$bdayleave','$blp_used','$earlyout','$eo_used','$slp','$slp_used','$addedby','$datenow')";
